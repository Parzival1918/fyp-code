...
# Calculate the density of water, potassium and chloride in the control region (CR):
...
PRINT ARG=n_potassium,n_chloride,n_water,res_potassium.bias,res_chloride.bias STRIDE=10000 FILE=CuMD.log
...
"""
    ...
    with open(filename, mode="w") as f:
        ...
        f.write("chloride: GROUP ATOMS=")
        f.write(",".join(str(x) for x in cl_ids))
        f.write("\n\n")
        ...
        f.write(f"n_chloride: CMUMD GROUP=chloride FIXED={fix} NSV=1 DCR={dcr} CRSIZE={crsize} WF=0.001 ASYMM=1\n")
        ...
        f.write(f"res_chloride: RESTRAINT ARG=n_chloride AT={conc} KAPPA=10000.0\n")
...
def main():
    ...
    for atom in atom_data:
        if atom["type"] == 1:
            ...
            cl_ids.append(atom["id"])

    # create plumed.in file
    print("Creating plumed input file... ", end="")
    create_plumed_file(plumed_file, k_ids, cl_ids, ow_ids, settings)
    print("done")
